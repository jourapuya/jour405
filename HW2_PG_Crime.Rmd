...
The general groups of all of the incidents seem to be accidents, assaults, auto crime, breaking and entering, homicide, robbery, sex offense and theft. Assault, B&E, and robbery are the groups with the most categories, though all of their categories stay in double digits by month, compared to categories that consistently see triple digits monthly. These groups make summarizing the data a bit more difficult because you have to add all of their categories together to get stats for one general categorical group. From a reporting standpoint, I would say the assault shootings definitely rise by a large percentage rate, though not by a large numerical value, but it is interesting to point out. 
...
Accidents, auto stolen, and theft all have about the highest average number of incidents per month. B&E Other, homicide, and residential robberies have the lowest average number of incidents per month. This tells us that a lot of high scale crime happens in the county, meaning crimes that involve some big tool like a car, besides something like someone stabbing somebody with a knife or breaking into a house with a crowbar. I don't really have questions about the categories, it seems plausible that a lot of bigger scale things like accidents or auto theft are highest because almost everyone owns and also needs a car at the same time.
...
```{r}
summer_crime_averages <- pg_crime_averages |> mutate(june_change = (`average_incidents`- `Jun`)/`Jun` * 100) |> mutate(july_change = (`average_incidents`- `Jul`)/`Jul` * 100) |> mutate(august_change = (`average_incidents`- `Aug`)/`Aug` * 100)
```
...
Almost every category has a summer average that is negative, meaning, without even looking at the numbers yet, the summer months are already bigger than the averages in many cases. It supports the theory that crime is more likely to occur in the summer months.
...
Crime reports of vehicle robberies experience nearly 40% difference in July compared to the average, according to Prince George's County statistics.

Many crimes in Prince George's County experience an resurgence during the middle of the year, but vehicle robberies are one of the largest affected categories. Crimes involving vehicles in any capacity are already more likely to take place more than any other crime, which puts many average working people at risk.
